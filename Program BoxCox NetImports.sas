proc transreg data=WORK.IMPORT ss2 details
              plots=(transformation(dependent) scatter
                    observedbypredicted);
   model BoxCox('net imports'n / lambda=-2 -1 -0.5 to 0.5 by 0.05 1 2
...
data work.transform;
	set WORK.IMPORT;
	'tr1_net imports'n='net imports'n-1;
run;
...
proc glmselect data=work.transform outdesign(addinputvars)=Work.reg_design 
	...
	model 'tr1_net imports'n='Real-World MPG'n / showpvalues selection=stepwise
...
proc reg data=Work.reg_design alpha=0.05 plots(only)=(diagnostics residuals 
	...
	model 'tr1_net imports'n=&_GLSMOD /;
	run;
quit;
